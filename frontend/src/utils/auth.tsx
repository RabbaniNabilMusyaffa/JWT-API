...
interface IAuthContext {
	...
	tryForgotPassword: (_email: string | undefined) => Promise<unknown>;
...
export const AuthProvider: FC<AuthProviderProps> = ({ children }) => {
	...
	const tryVerify = async (_token: string) => {
		const body = {
			token: _token,
		};
	...
	const tryForgotPassword = async (_email: string | undefined) => {
		const body = {
			email: _email,
		};
		...
		return await new Promise((resolve, reject) => {
			axios
				.post(`${API}/auth/forgotpassword`, body, config)
				.then(res => {
					resolve(res);
				})
	...
	const requestFeed = async () => {
		return await new Promise((resolve, reject) => {
			axios
				.get(`${API}/posts/following`, config)
				.then(res => {
					resolve(res);
				})
				.catch(error => {
